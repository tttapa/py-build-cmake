...
| `generator` | CMake generator to use, passed to the configuration step, as `-G <?>`. If Ninja is used, and if it is not available in the system PATH, it will be installed automatically as a build dependency.<br/>For example: `generator = "Ninja Multi-Config"` | string | `ENV{CMAKE_GENERATOR}` |
...
| `options` | Extra options passed to the configuration step, as `-D<option>=<value>`.<br/>Note that setting `CMAKE_OSX_DEPLOYMENT_TARGET` here is not supported, see <project:../usage/faq.md#how-to-set-the-minimum-supported-macos-version>.<br/>For example: `options = {"WITH_FEATURE_X" = true}` | dict (CMake) | `{}` |
...
| `sabi_library` | Python library file (.so on Linux, .lib on Windows) for the stable ABI. Used to set the `Python3_SABI_LIBRARY` CMake artifact, see <https://cmake.org/cmake/help/latest/module/FindPython3.html#artifacts-specification>.<br/>Absolute or relative to current configuration file. | filepath | `none` |
